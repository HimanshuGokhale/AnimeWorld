...
@media screen and (max-height: 450px) {
  ...
  <body >
   <c:set var="sesorder" value="${sesorder}"/>
      <c:choose>
          <c:when test="${sesorder=='success'}">
             <script type="text/javascript">
             swal("!!!Order Cancel Successfully!!!", "","success");
             </script>
          </c:when>
      </c:choose>
      <c:remove scope="session" var="sesorder"/>
					     ...
					     <table class="table"  id="example" class="table table-striped table-bordered">
 							 <thead>
  							  ...
  							  <tbody>
  							   ...
  							   <td>${list.bill}</td>
  							   <td>${list.status}</td>
  							   <td><a href="/AnimeWorldSpringMVC/cancel/${list.order_id}">Cancel</a></td>
  							   <td><a href="/AnimeWorldSpringMVC/view/${list.order_id}">View</a></td>
